...
class Transolver_conv_proj(BaseModel):
    def __init__(
        self,
        space_dim=1,
        n_layers=5,
        ...
        super(Transolver_conv_proj, self).__init__()
        ...
        if self.unified_pos:
            ...
            self.lifting = Lifting(in_channels=fun_dim + space_dim, out_channels=n_hidden, n_dim=1)
    ...
    def forward(self, data):
        ...
        if fx is not None:  # 不执行
            ...
            fx = self.lifting(x.permute(0,2,1)).permute(0,2,1)  # torch.Size([1, 3586, 256])
